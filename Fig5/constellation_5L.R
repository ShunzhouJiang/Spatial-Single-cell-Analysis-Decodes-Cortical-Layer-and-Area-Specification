...
dir = '/Users/kylecoleman/data/walsh/all/clustering2/constellation/genes1000/'

for (cutoff in c(0.02)){
  col_classes <- c("character", "character", "character")  
  cells.cl.df = read.csv(paste(dir,'obs.csv',sep=''), header = TRUE, colClasses = col_classes, row.names = 1)
  cluster_size = table(cells.cl.df$group)
  cells.cl.df$cell_name = rownames(cells.cl.df)
  rd.dat <- py_load_object(paste(dir,'red.pickle',sep=''))
  ...
  np <- import("numpy")
  knn.result = list()
  knn.result[[1]] <- np$load(paste(dir,'indices.npy',sep=''))
  knn.result[[1]] = knn.result[[1]] + 1
  knn.result[[2]] <- np$load(paste(dir,'distances.npy',sep=''))
  groups.col = 'group'
